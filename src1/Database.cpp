...
std::vector<std::vector<ClkRgn*>> Database::deepCopy(const std::vector<std::vector<ClkRgn*>>& original) {
    ...
    for (const auto& row : original) {
        ...
        for (const auto& elem : row) {
            newRow.push_back(new ClkRgn(*elem)); // 分配新内存并拷贝倄1�7
        }
...
void Database::refineDBsites(){
    //遍历db.instances去修正db.sites，主要是site对应的pack有问预1�7
    this->smartsites.resize(sitemap_nx, vector<Site *>(sitemap_ny));
    for (int row = 0;row < sitemap_nx;row++){
        for (int col = 0;col <sitemap_ny;col++){
            smartsites[row][col] = sites[row][col];
            // cout<<"refine sites"<<endl;
        }
    }
