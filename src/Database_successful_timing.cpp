...
//            先判断��取的PLB是否为空,或��要不要找内容最多的PLB
    ...
    //上面是寻找最大独立集集合，这段用多线程
    int max_idpset = 0;
    int idx_max_idpset = 0;
    for (int s= 0;s<IndependLUTs.size();s++){//遍历这个时钟区域扢�有的独立雄1�7
        if (IndependLUTs[s].size()>max_idpset){
            max_idpset = IndependLUTs[s].size();
            idx_max_idpset = s;

        }
    }
    vector<pair<Instance *,Instance *>> LUTs = IndependLUTs[idx_max_idpset];
    cout<<"successful for find independent set"<<endl;
