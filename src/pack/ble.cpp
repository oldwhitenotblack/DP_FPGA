...
void BLE::AddLUT(Instance* const inst) {
    assert(inst->IsLUT() && LUTs.size() < 8);
    LUTs.push_back(inst);
}

void BLE::AddFF(Instance* const inst) {
    assert(inst->IsFF() && FFs.size() < 8);
    // clk, sr, ce
    // cout << FFs.size() << endl;
    if (FFs.size() != 0) {//也就是看是否这个ble已经加了dff，加了就判断第二个dff是不是跟第一个一样否则直接加
        if (inst->pins[database.clkPinIdx]->net != NULL) {
            if (type.clk != NULL) {
                // cout << this->bank << ',' << type.clk->name<< ',' << inst->pins[database.clkPinIdx]->net->name << endl;
            }
            assert(type.clk == inst->pins[database.clkPinIdx]->net);
        } else if (inst->pins[database.srPinIdx]->net != NULL) {
            assert(type.sr == inst->pins[database.srPinIdx]->net);
        }
    } else if (FFs.size() == 0){
        // cout << inst->pins[database.clkPinIdx]->net << ',' << inst->pins[database.srPinIdx]->net << endl;
        type.clk = inst->pins[database.clkPinIdx]->net;
        type.sr = inst->pins[database.srPinIdx]->net;
    }
    type.ce.push_back(inst->pins[database.cePinIdx]->net); //存dff后面输出的cePin所连接的网表
    // FF, nets
    FFs.push_back(inst);
}
void BLE::Remove(Instance* inst) {
    if (inst->master->name == Master::SEQ) {
        this->FFs.erase(remove(this->FFs.begin(), this->FFs.end(), inst), this->FFs.end());
    } else {
        this->LUTs.erase(remove(this->LUTs.begin(), this->LUTs.end(), inst), this->LUTs.end());
    }
}
bool BLE::IsLUTCompatWith(BLE* rhs) {
    return LUTs.size() == 0 || rhs->LUTs.size() == 0 ||
           (LUTs.size() == 1 && rhs->LUTs.size() == 1 &&
            (LUTs[0]->pins.size() + rhs->LUTs[0]->pins.size() <= 7 || database.IsLUTCompatible(*LUTs[0], *rhs->LUTs[0])));
...
void BLE::CheckBles(const vector<BLE*>& bles) {
    ...
    if (!existMiss) cout << "BLE::CheckBles: there is no missed inst" << endl;
}