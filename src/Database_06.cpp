...
bool Database::isSameNet(shared_ptr<Instance> a,shared_ptr<Instance> b){
    for (auto inst:a->ConnInsts){
        if (shared_ptr<Instance>(inst)==b){
            return true;
        }
    }
    return false;
...
void Database::dplace(){
    ...
    for (auto clkrgns:database.clkrgns){
        int cnt = 0;
        for (auto clkrgn:clkrgns){
            vector<vector<pair<shared_ptr<Instance>,shared_ptr<Instance>>>> IndependLUT = test();
            vector<vector<pair<shared_ptr<Instance>,shared_ptr<Instance>>>> IndependLUTs = getIndependentLUTs(shared_ptr<ClkRgn>(clkrgn));
            cnt ++;
            cout <<"cnt for getIndependentset:"<<cnt<<endl;
        ...
        for (int j = 0;j<5;j++){
            for (int row = clkidx_row[i];row < clkidx_row[i+1];row++)
                for (int col = clkidx_col[j];col < clkidx_col[j+1];col++){
                    clkrgns[i][j]->smartsites.push_back(shared_ptr<Site>(sites[row][col]));
                }
        }
    }
}
