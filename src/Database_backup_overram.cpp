...
//            先判断��取的PLB是否为空,或��要不要找内容最多的PLB
    ...
    for (auto LUTs :IndependLUTs){
        vector<vector<pair<int, long>>> allCandSites(LUTs.size(),vector<pair<int, long>>(LUTs.size() +recEmptySpace.size(), make_pair(-1,0)));  // (site, score),这是丢�个无向图
        ...
        for (int i = 0;i<LUTs.size();i++){
            for (int j =0 ;j<recEmptySpace.size();j++){
                ...
                if (lvpair.first->master->name == Master::LUT6)
                    ...
                    cost = calLUTSwapCost(lvpair.first,emptypair.first)+calLUTSwapCost(lvpair.second,emptypair.second)+COST_BASE;
                }
        ...
        for (auto pair : pairs) {
            ...
            if (pair.first.first->master->name == Master::LUT6 && pair.second.first->master->name == Master::LUT6){
                ...
                if (pair.first.first->master->name == Master::LUT6){
                    ...
                    cout<<"swapLUT6withNLUT6 cost:"<<calLUT6wNLUT6Cost(pair.first.first,pair.second.first,pair.second.second)<<endl;
                }
                else{
                    if (pair.second.first->master->name == Master::LUT6){
                        ...
                        cout<<"swapLUT6withNLUT6 cost:"<<calLUT6wNLUT6Cost(pair.second.first,pair.first.first,pair.first.second)<<endl;
                    }
                    else{
                        SwapInstMap(pair.first.first, pair.second.first);
