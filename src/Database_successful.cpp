...
//            int mid = PLBs.size()/2;
            ...
            int mid = PLBs.size()/2;
            cout<<"mid:"<<mid<<"PLB SIZE:"<<PLBs.size()<<endl;
            ...
            for (auto LUTs:IndependLUTs){//遍历这个时钟区域扢�有的独立雄1�7
                ...
                if (isSuccessfulBipartite) {
                    for (auto pair : pairs) {
                        ...
                        if (pair.first.first->pack->site->x == 55 && pair.first.first->pack->site->x == 124){
                            cout<<"test for over cap"<<endl;
                        }
                        if (pair.second.first->pack->site->x == 55 && pair.second.first->pack->site->x == 124){
                            cout<<"test for over cap"<<endl;
                        }
                        if (pair.first.second->pack->site->x == 55 && pair.first.second->pack->site->y == 124){
                            cout<<"test for over cap"<<endl;
                        }
                        if (pair.second.second->pack->site->x == 55 && pair.second.second->pack->site->y == 124){
                            cout<<"test for over cap"<<endl;
                        }
                    }
...
void Database::SwapInstMap(Instance *a,Instance *b){
    ...
    b->bankz = a->bankz;
    //上面交换inst，下面交换pack
    Instance* tmpidx_Inst = a->pack->instances[a->packz];
    a->pack->instances[a->packz] = b->pack->instances[b->packz];
