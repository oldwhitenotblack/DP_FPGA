...
//PLB内的没必要交换，从中间向外扩
            for (int i = 0;i < 8;i++){//遍历中间PLB的所有LUT
                ...
                if (luta->master->name == Master::LUT6||lutb->master->name == Master::LUT6){
                    if (luta ->packz+1 != lutb->packz){
                        cout <<"test for LUT6"<<endl;
                    }
                }
                ...
                for (auto s:PLBs){//judge if the LUT is independent
                    ...
                    for (int j = 0;j < 8;j++){
                        ...
                        if (lutc->master->name == Master::LUT6||lutd->master->name == Master::LUT6){
                            if (lutc ->packz+1 != lutd->packz){
                                cout <<"test for LUT6"<<endl;
                            }
                        }
                        int isIndepend = 1;
