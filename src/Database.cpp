...
#define MAX_TIMES_OF_SELECT_PLB_FOR_BIPARTITE 10
#define MIN_SIEZE_OF_LUT_FOR_BIPARTITE 4
#define COST_BASE 10000
#define MAX_FIXED 4
#include "Database.h"
using namespace db;
...
void Database::swapLUT6withNLUT6(Instance* lut6,Instance* nlut6a,Instance* nlut6b){//这个函数有问题
...
bool Database::isIndependent(vector<pair<Instance *,Instance *>> independentSet){
    for (size_t t = 0;t<independentSet.size();t++){
        pair<Instance *,Instance *> pairlut1 = independentSet[t];
        for (size_t i = t+1;i<independentSet.size();i++){
            pair<Instance *,Instance *> pairlut2 = independentSet[i];
            if (isSameNet(pairlut1.first,pairlut2.first)|| isSameNet(pairlut1.first,pairlut2.second)||
                    isSameNet(pairlut1.second,pairlut2.first)||isSameNet(pairlut1.second,pairlut2.second)){
                return false;
            }
        }
    }
...
//            先判断��取的PLB是否为空,或��要不要找内容最多的PLB
            for (int i = 0; i < MAX_TIMES_OF_SELECT_PLB_FOR_BIPARTITE; i++) {
                ...
                for (int j = 0; j < 8; ++j) {
                    if (PLBs[mid]->pack->instances[2 * j] != nullptr ||
                        PLBs[mid]->pack->instances[2 * j + 1] != nullptr) {
                        ++cntLUT;
                    }
                }
            ...
            vector<pair<Instance *, Instance *>> recEmptySpace;
            for (int i = 0; i < 8; i++) {//遍历中间PLB的所有LUT
                ...
                if (luta == NULL && lutb == NULL) {
                    Instance *tmp1 = new Instance();
                    tmp1->x = PLBs[mid]->x;
                    tmp1->y = PLBs[mid]->y;
                    tmp1->z = i;
                    tmp1->pack = PLBs[mid]->pack;
                    tmp1->packz = 2 * i;
                    tmp1->bankz = i / 4;
                    Instance *tmp2 = new Instance();
                    tmp2->x = PLBs[mid]->x;
                    tmp2->y = PLBs[mid]->y;
                    tmp2->z = i;
                    tmp2->pack = PLBs[mid]->pack;
                    tmp2->packz = 2 * i + 1;
                    tmp2->bankz = i / 4;
                    recEmptySpace.push_back(make_pair(tmp1, tmp2));
                    continue;
                } else {
                    if (luta == NULL || lutb == NULL) continue;
                }
                            ...
                            if (lut.first != lut.second) {
                                if (lutc->master->name == Master::LUT6) {
                                    ...
                                    if (isSameNet(lut.first, lutc) || isSameNet(lut.first, lutd) ||
                                        isSameNet(lut.second, lutd) || isSameNet(lut.second, lutc)) {
                                        isIndepend = 0;
                                        break;
                                    }
                                }
            ...
            cout << "successful for init indepentdent set" << endl;
            if (IndependLUTs.empty()) continue;
            if (isIndependent(IndependLUTs[0])) cout << "independent correct" << endl;

            int idx_best_set = 0;
            int min_cost = INT_MAX;
            vector<pair<int, long>> best_res;
            for (auto LUTs: IndependLUTs) {
                vector<vector<pair<int, long>>> allCandSites(LUTs.size(),
                                                             vector<pair<int, long>>(LUTs.size() + recEmptySpace.size(),
                                                                                     make_pair(-1,
                                                                                               0)));  // (site, score),这是丢�个无向图
                ...
                for (int row = 0; row < LUTs.size(); row++) {
                    for (int col = row + 1; col < LUTs.size(); col++) {
                        ...
                        if (LUTs[row].first->master->name == Master::LUT6 &&
                            LUTs[col].first->master->name == Master::LUT6) {
                            cost += calLUTSwapCost(LUTs[row].first, LUTs[col].first) + COST_BASE;
//                    cost += COST_BASE-calLUTSwapCost(LUTs[row].first,LUTs[col].first);

                        } else {
                            if (LUTs[row].first->master->name == Master::LUT6) {
                                cost += calLUT6wNLUT6Cost(LUTs[row].first, LUTs[col].first, LUTs[col].second) +
                                        COST_BASE;
//                        cost += COST_BASE-calLUT6wNLUT6Cost(LUTs[row].first,LUTs[col].first,LUTs[col].second);

                            } else {
                                if (LUTs[col].first->master->name == Master::LUT6) {
                                    cost += calLUT6wNLUT6Cost(LUTs[col].first, LUTs[row].first, LUTs[row].second) +
                                            COST_BASE;
//                            cost += COST_BASE-calLUT6wNLUT6Cost(LUTs[col].first,LUTs[row].first,LUTs[row].second);

                                } else {
                                    cost += calLUTSwapCost(LUTs[row].first, LUTs[col].first) +
                                            calLUTSwapCost(LUTs[row].second, LUTs[col].second) + COST_BASE;
//                            cost += COST_BASE-calLUTSwapCost(LUTs[row].first,LUTs[col].first)- calLUTSwapCost(LUTs[row].second,LUTs[col].second);
                ...
                //这里把PLB中的空位置也加入到无向图右顶点集中，用个数据结构记录这些空位置原来的pack和pack->instances
                for (int i = 0; i < LUTs.size(); i++) {
                    for (int j = 0; j < recEmptySpace.size(); j++) {
                        auto lvpair = LUTs[i];
                        auto emptypair = recEmptySpace[j];
                        int cost = 0;
                        if (lvpair.first->master->name == Master::LUT6)
                            cost = calLUT6wNLUT6Cost(lvpair.first, emptypair.first, emptypair.second) + COST_BASE;
                        else {
                            cost = calLUTSwapCost(lvpair.first, emptypair.first) +
                                   calLUTSwapCost(lvpair.second, emptypair.second) + COST_BASE;
                        }
                        allCandSites[i][LUTs.size() + j] = {LUTs.size() + j, cost};
                    }
                }
                cout << "successful for init graph" << endl;
                vector<pair<int, long>> res;
                long cost = 0;
                MinCostBipartiteMatching(allCandSites, allCandSites.size(), allCandSites[0].size(), res, cost);
                cout << "bipartite onece" << endl;
                if (cost < min_cost) {
                    min_cost = cost;
            ...
            cout << "res size:" << best_res.size() << endl;
            cout << "res cost" << min_cost << endl;
            ...
            cout<<"successful add pair"<<endl;
            if (isSuccessfulBipartite) {
                int cost = 0;
                for (auto pair: pairs) {
                    if (pair.second.first->id == -1) {
                        if (pair.first.first->master->name == Master::LUT6) {
                            swapLUT6withNLUT6(pair.first.first, pair.second.first, pair.second.second);
                            cout<<"successful for swapLUT6withNLUT6 when lut6 and null"<<endl;
                        } else {
                            SwapInstMap(pair.first.first, pair.second.first);
                            SwapInstMap(pair.first.second, pair.second.second);
                            cout<<"successful for swap when lut and null"<<endl;
                        }
                        continue;
                    }
                    if (pair.first.first->master->name == Master::LUT6 &&
                        pair.second.first->master->name == Master::LUT6) {
                        swapLUT6withLUT6(pair.first.first, pair.second.first);
                        cost += calLUTSwapCost(pair.first.first, pair.second.first);
                        cout << "swapLUT6withLUT6 cost:" << calLUTSwapCost(pair.first.first, pair.second.first) << endl;
                    } else {
                        if (pair.first.first->master->name == Master::LUT6) {
                            swapLUT6withNLUT6(pair.first.first, pair.second.first, pair.second.second);
                            cost += calLUT6wNLUT6Cost(pair.first.first, pair.second.first, pair.second.second);
                            cout << "swapLUT6withNLUT6 cost:"
                                 << calLUT6wNLUT6Cost(pair.first.first, pair.second.first, pair.second.second) << endl;
                        } else {
                            if (pair.second.first->master->name == Master::LUT6) {
                                swapLUT6withNLUT6(pair.second.first, pair.first.first, pair.first.second);
                                cost += calLUT6wNLUT6Cost(pair.second.first, pair.first.first, pair.first.second);
                                cout << "swapLUT6withNLUT6 cost:"
                                     << calLUT6wNLUT6Cost(pair.second.first, pair.first.first, pair.first.second)
                                     << endl;
                            } else {
                                SwapInstMap(pair.first.first, pair.second.first);
                                cout << "swap cost:" << calLUTSwapCost(pair.first.first, pair.second.first) << endl;
                                cost += calLUTSwapCost(pair.first.first, pair.second.first);
                                SwapInstMap(pair.first.second, pair.second.second);
                                cout << "swap cost:" << calLUTSwapCost(pair.first.second, pair.second.second) << endl;
                                cost += calLUTSwapCost(pair.first.second, pair.second.second);
                            }
                        }
                    }
                }
...
//加个搜索inst的关键路径的函数，返回inst
vector<Instance*> Database::searchTimingInst(Instance *a){
    //遍历inst的所有pin，然后找pin所处的线网，问题是一个pin只处于一个线网吗
    vector<Instance*> timingInsts;
    for (auto p:a->pins){
        if (p->isTiming){//找这个pin连接的所有inst
            timingInsts.push_back(p->net->pins[0]->instance);
        }
    }
//    if (!timingInsts.empty()) cout<<" find timing insts"<<endl;
    return timingInsts;
}
...
int Database::calLUTSwapCost(Instance *a,Instance *b){
     ...
     * 2、计算交换后的距禄1�7
     * 3、计算交换后PLB的pin密度
     * 4、有个问题：用二分匹配的话，移动后的代价是��线长还是局郄1�7
     */
    vector<int> oriDist1;
    vector<int> oriDist2;
    vector<int> aftDist1;
    ...
    if (a->id == -1 && b->id == -1){
        ...
        if (a->id == -1){
            vector<Instance*> timingInsts2 = searchTimingInst(b);
            if (!timingInsts2.empty()){
                for (auto inst:timingInsts2){
                    int px2 = inst->x;
                    int py2 = inst->y;
                ...
                for (int i = 0;i<oriDist2.size();i++){
                    cost += 5*(aftDist2[i]-oriDist2[i]);
                }
            }
            oriDist2.clear();
            aftDist2.clear();
            for (auto inst:b->ConnInsts){
                if (find(timingInsts2.begin(),timingInsts2.end(),inst)!=timingInsts2.end()) continue;
                int px2 = inst->x;
                int py2 = inst->y;
                oriDist2.push_back(calMhatton(x2,y2,px2,py2));
                aftDist2.push_back(calMhatton(x1,y1,px2,py2));
        ...
        if (b->id == -1){
            vector<Instance*> timingInsts1 = searchTimingInst(a);
            if (!timingInsts1.empty()){
                for (auto inst:timingInsts1){
                    int px1 = inst->x;
                    int py1 = inst->y;
                ...
                for (int i = 0;i<oriDist1.size();i++){
                    cost += 5*(aftDist1[i]-oriDist1[i]);
                }
            }
            oriDist1.clear();
            aftDist1.clear();
            for (auto inst:a->ConnInsts){
                if (find(timingInsts1.begin(),timingInsts1.end(),inst)!=timingInsts1.end()) continue;
                int px1 = inst->x;
                int py1 = inst->y;
                oriDist1.push_back(calMhatton(x1,y1,px1,py1));
                aftDist1.push_back(calMhatton(x2,y2,px1,py1));
...
int Database::calLUT6wNLUT6Cost(Instance *lut6,Instance *nlut6a,Instance *nlut6b){
    ...
    vector<Instance*> timingInsts1 = searchTimingInst(nlut6b);
    if (!timingInsts1.empty()) {
        for (auto inst:timingInsts1){
            int px2 = inst->x;
            int py2 = inst->y;
